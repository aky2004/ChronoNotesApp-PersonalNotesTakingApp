...
import { addUser, setToken } from '../Redux/AuthSlice'; // add setToken if you want to store token in redux
...
export default function Login() {
  ...
  const handleSubmit=async(e)=>{
        e.preventDefault()
        try {
          // use relative path so baseURL from ApiEndPoint is used
          ...
          if (response.success) {
             toast.success(response.message)
              ...
              // optionally store token in redux/localStorage action
              if (response.token) {
